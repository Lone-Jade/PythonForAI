    ...
    def plot_route(self, route, best_dist, save_path="."):
        ...
        if save_path:
            ...
            save_path = os.path.join(
                save_path, f"tsp_best_route_distance_{best_dist:.2f}_2-opt.png"
            )
...
if __name__ == "__main__":
    ...
    tsp = TwoOptTSP(cities, coordinates, origin="北京")
    best_route, best_dist = tsp.solve()

    # 测试基本信息
    print(f"城市数量: {tsp.num_cities}")
    print("城市列表:")
    for i, name in enumerate(tsp.cities):
        print(pad_string(name, 10), end="")
        if (i + 1) % 10 == 0:
            print()
    print()
    print("=" * 60)
    print(f"算法运行时间: {time.time() - start_time:.2f} 秒")
